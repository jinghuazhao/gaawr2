...
pkgs <- c("EnsDb.Hsapiens.v75","GMMAT","HardyWeinberg","MCMCglmm","SNPassoc","biomaRt","gap","haplo.stats","pedigreemm",
...
snpCols <- grep("^rs", colnames(asthma), value=TRUE)[1:3]
...
We then turn to genetic models for a specific SNP,
...
SNPassoc::association(cc ~ dominant(rs4490198)*factor(smoke), data=asthma.snps)
...
An overview on the proteomics is found from the relevant paper @suhre20.
...
```r
assoc <- SNPassoc::WGassociation(cc, data=asthma.snps, mc.cores=1)
assoc %>%
as.data.frame() %>%
dplyr::select(-comments) %>%
knitr::kable(caption="SNP association")
assoc.adj <- SNPassoc::WGassociation(cc ~ country + smoke, asthma.snps, mc.cores=1)
assoc.adj %>%
as.data.frame() %>%
dplyr::select(-comments) %>%
