...
The package gathers information, meta-data and scripts in a recent Henry-Stewart talk.
...
We more often embed R in a Linux script as shown below,

```{bash linux}
# quoting code with delimiters
R --no-save -q <<END
print("Hello, world!")
END
...
More generally as shown in the example, one can take advantage of powerful data handling facilities
in the Linux environment, through either itself or software followed by their counterparts in R with
options to feed back to the Linux envornment again for further use.

Further to this point, R could be a powerful part in a workflow, e.g., an article has been curated
in **pQTLtools** involving **snakemake**, <https://jinghuazhao.github.io/pQTLtools/articles/snakemake.html>.

# Language elements

Basic data manipulation can be seen with
...
knitr::kable(mean_values,caption="Mean value by gender and diabetes category")
...
We've explored various aspects of genetic association analysis in R, particularly in the context of computing systems
like Linux. While these serve as a solid foundation, a more in-depth data analysis coupled with development is surely
fruitful.
