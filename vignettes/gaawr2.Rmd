...
pkgs <- c("EnsDb.Hsapiens.v75","GMMAT","HardyWeinberg","MCMCglmm","biomaRt","gap","haplo.stats","pedigreemm","httr","jsonlite","knitr")
...
The operators are intuitive in many ways, but we would like to highlight two of them,

- `|>` which is a pipe operator enabling many commands piped in the same way as in Linux.
- `%>%` popularized from R magritter package, essentially another yet more powerful pipe command.
...
Note the scope operator (`::`) is useful since user executes command from a particular package without loading it, which is usually faster. With growing popularity of GitHub and also due to its less strict control over format from CRAN, it is customarily used but we will leave this aspect later on.
...
- Omics. Proteomics such as `MsCoreUtils`, `MSStats`, `scp`.
...
Note that for creation of the GitHub repository, an SSH key is already in place. In order for `pkgdown.sh` to function well, all required files such as `nature-genetics.csl` need to be available.
...
We carry on adding files such as `NEWS.md` and `_pkgdown.yml`. In line with various analyses we have covered, their associate packages are also added to the suggested list of packages in DESCRIPTION:
