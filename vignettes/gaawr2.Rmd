...
pkgs <- c("EnsDb.Hsapiens.v75","ensembldb","GMMAT","HardyWeinberg","MCMCglmm","SNPassoc","biomaRt",
           ...
           "dplyr","ggplot2","httr","jsonlite","kableExtra","knitr")
...
The package intends to gather information, meta-data and scripts in genetic association analysis as a companion to a Henry-Stewart talk.

It is adapted from earlier package development work, particularly pQTLdata, <https://jinghuazhao.github.io/pQTLdata/>.
...
```{r ops, fig.cap="Average Horsepower by Cylinder Count (mpg > 20)", fig.height=6, fig.with=8}
sumstats <- mtcars %>%
  dplyr::filter(mpg > 20) %>%
  dplyr::group_by(cyl) %>%
...
ggplot2::labs(title = "", x = "Number of Cylinders", y = "Average Horsepower") +
